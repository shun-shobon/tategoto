...
録音開始時にマイク入力を開始する。マイク権限が拒否されている場合や入力 stream を開始
できない場合は `stopped_with_error` にする。SpeechTranscriber / SpeechAnalyzer は
旧 Speech authorization API を使わず、入力 audio buffer と AssetInventory の
model asset を前提に動かす。
